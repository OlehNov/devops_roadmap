...
CMD ["dockerize", "-wait", "tcp://db:3306", "-timeout", "60s", "sh", "-c", "python src/eventlogs/create_eventlog_db_script.py && python src/manage.py makemigrations && python src/manage.py migrate && python src/manage.py runserver 0.0.0.0:8000"]
